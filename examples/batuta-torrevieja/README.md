# La Batuta de Torrevieja v0.2

La Batuta de Torrevieja v0.2 es el módulo premium independiente de `map-gesture-controls`: una experiencia de destino donde el usuario no sólo mueve un mapa, sino que compone una ruta viva con sus manos.
...
Todo se configura en [`batuta-torrevieja.config.json`](batuta-torrevieja.config.json):
...
```bash
batuta-torrevieja.html?config=<url-del-json>
```

Forzar modo escaparate:

```bash
batuta-torrevieja.html?kiosk=1
```
...
npm install
npm run dev
# http://localhost:5173/map-gesture-controls/batuta-torrevieja.html
```

El build de demos (`npm run docs:build-demos`) incluye `batuta-torrevieja.html`, por lo que al desplegar GitHub Pages queda en:

```text
/map-gesture-controls/demo/batuta-torrevieja.html
```
