/**
 * La Batuta de Torrevieja — personalization contract.
 *
 ...
 */
import defaultConfig from './batuta-torrevieja.config.json';
  ...
  if (configUrl) {
    try {
      ...
      if (res.ok) {
        ...
        console.warn(
          `[batuta-torrevieja] config fetch failed (${res.status}), using bundled demo config`,
        );
      }
    } catch (err) {
      console.warn(
        '[batuta-torrevieja] config fetch error, using bundled demo config',
        err,
      );
    }
  }
